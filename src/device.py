...
class CompiledShuttlingSchedule:
    ...
    def get_frames(self, frame_timing: int = 100) -> list[Frame]:
        ...
        for t in range(0, max(instr.duration for instr in self.instructions) + frame_timing, frame_timing):
            active_instrs = []
            ...
            for instr_idx in active_instrs:
                ...
                if isinstance(instr, SingleQubitInstruction):
                    if isinstance(instr, Shuttle):
                        ...
                        x1,y1 = instr.coords
                        if instr.start_component == DeviceComponent.READOUT:
                            x1 -= 0.2
                            y1 -= 0.2
                        elif instr.start_component == DeviceComponent.INTERACTION_ZONE:
                            x1 += 0.2
                            y1 += 0.2
                            if (x1, y1) in qubit_positions.items():
        ...
        while remaining_ancilla:
            ...
            def add_state(state, state_new, transition_cost):
                ...
                if state_new in cost_to_node:
                    print(f'\t\ttalready seen, cost {cost_to_node[state_new]}, new cost {cost_to_node[state] + transition_cost}')
                if cost_to_node.get(state_new, MAX_T) > cost_to_node[state] + transition_cost:
                    ...
                    print('\t\testimated cost:', node_heuristic_vals[state_new])
                else:
                    print('\t\tpass')
            ...
            while frontier:
                ...
                print(f'POP {(state.x, state.y, str(state.component), state.cur_task)} with cost {node_heuristic_vals[state]} ({cost_to_node[state]} + {heuristic(state)})')
                if node_heuristic_vals[state] - cost_to_node[state] == 0:
                    ...
                    max_path_time = max(max_path_time, cost_to_node[state])
                    break
