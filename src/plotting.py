...
def plot_device_snapshot(
    ...
    ax.clear()
    ax.add_patch(mpl.patches.Rectangle((-1.05, -1.05), dev.w+0.1, dev.h+0.1, facecolor='lightgray', zorder=0.5))
    # Draw device
    ...
    for q,coords in frame.qubit_positions.items():
        ...
        ax.add_patch(mpl.patches.Circle(coords, radius=0.1, facecolor=fc, edgecolor=color, linewidth=1, zorder=2))
    ...
    for qbs in frame.twoq_gates:
        ...
        if coords not in seen_coords:
            ax.add_patch(mpl.patches.Rectangle((x-0.25, y-0.45), 0.2, 0.4, edgecolor='limegreen', facecolor='none', linewidth=2, zorder=3))
...
def plot_transition(
    ...
    for q,positions in qubit_loc_history.items():
        ...
        # ax.annotate('', xytext=pos, xy=pos2, arrowprops=dict(arrowstyle='->, head_width=0.5, head_length=0.35', linewidth=2, color=color), zorder=2.5)
